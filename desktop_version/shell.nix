...
in
  pkgs.callPackage (
    {smpeg2, mkShell, cmake, pkgsStatic, SDL2, SDL2_mixer, automake, fribidi, pkgconfig, ninja, zlib, libpng, libicns, imagemagick, curl}:
    (mkShell.override { inherit stdenv; }) (let sdl = (SDL2.override {x11Support = stdenv.isLinux;}).overrideAttrs (oldAttrs: {
      ...
      buildInputs = if stdenv.targetPlatform.isWindows then [
        ...
        zlib
        libpng
      ] else if android then [ pkgsNative.android-studio ] else [ SDL2 SDL2_mixer fribidi zlib libpng curl ];
