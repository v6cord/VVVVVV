        ...
        else if (game.currentmenuname == "loadcustomtrial")
        {
            if (ed.customtrials.size() == 0) {
                ...
                if ((game.currentmenuoption + 1) > (int)game.customtrialstats.size()) {
                    ...
                    dwgfx.Print( 110, 65, game.timetstring(game.customtrialstats[game.currentmenuoption].time), tr, tg, tb);
                    ...
                    dwgfx.Print( 170, 65, "PAR TIME    " + game.partimestring(), tr, tg, tb);
        ...
        else if (game.currentmenuname == "graphicoptions")
        {
          if (game.currentmenuoption == 0)
          {
              ...
              if(game.stretchMode == 6){
                dwgfx.Print( -1, 85, "Current mode: 4X", tr, tg, tb, true);
              }else if (game.stretchMode == 5){
                dwgfx.Print( -1, 85, "Current mode: 3X", tr, tg, tb, true);
              }else if (game.stretchMode == 4){
                dwgfx.Print( -1, 85, "Current mode: 2X", tr, tg, tb, true);
              }else if (game.stretchMode == 3){
                dwgfx.Print( -1, 85, "Current mode: 1X", tr, tg, tb, true);
...
void gamerender(Graphics& dwgfx, mapclass& map, Game& game, entityclass& obj, UtilityClass& help)
{
    ...
    if (map.roomtexton)
    {
        ...
        {
            dwgfx.Print(map.roomtext[i].x*8 + map.roomtext[i].subx, (map.roomtext[i].y*8) + map.roomtext[i].suby, map.roomtext[i].text, 196, 196, 255 - help.glow);
        }
    }
...
void towerrender(Graphics& dwgfx, Game& game, mapclass& map, entityclass& obj, UtilityClass& help)
{
    ...
    if (map.roomtexton)
    {
        ...
        for (size_t i = 0; i < map.roomtext.size(); i++)
        {
            dwgfx.Print(map.roomtext[i].x*8 + map.roomtext[i].subx, (map.roomtext[i].y*8) + map.roomtext[i].suby - map.ypos, map.roomtext[i].text, 196, 196, 255 - help.glow);
        }
    }
